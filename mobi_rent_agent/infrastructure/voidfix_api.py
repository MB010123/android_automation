        ...
        *,
        ...
        try:
            ...
            return SmsSendResult(
                success=False,
                to_number=to_number,
                error=_redact(f"unknown: timeout after provider request: {exc}", self._api_key),
        ...
        return interpret_send_response(
            response,
            to_number,
...
def extract_send_message_row(body: object | None) -> dict | None:
    if body is None:
        return None
    try:
        messages = _coerce_message_list(body)
...
def interpret_send_response(
    ...
    *,
    ...
    if not message_id:
        logger.info("SMS provider success lacked a message ID (HTTP %s)", status)
        return SmsSendResult(
            success=False,
            to_number=to_number,
            status_code=status,
    ...
    accept_row = extract_send_message_row(body)
    return SmsSendResult(
        success=True,
        to_number=to_number,
        status_code=status,
