...
def load_voidfix_sim_slots(path: str | Path | None) -> dict[int, int]:
    """Optional per-slot VoidFix SIM selector (1 or 2). Never sent unless enabled."""
    if path is None:
        return {}
    file_path = Path(path)
    if not file_path.exists():
    ...
    for key, value in raw.items():
        if not isinstance(value, dict):
            continue
        sim = value.get("sim_slot", value.get("simSlot"))
        if sim is None:
            continue
        slot_id = int(key)
        sim_slot = int(sim)
        if sim_slot not in {1, 2}:
            raise VoidFixDeviceMapError(f"slot {slot_id} sim_slot must be 1 or 2")
        slots[slot_id] = sim_slot
    return slots
